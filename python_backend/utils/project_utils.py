...
class ProjectService:
    ...
    async def list_user_projects(self, access_token: str, user_id: str) -> List[ProjectListItem]:
        ...
        try:
            ...
            repositories = await github_service.get_user_repositories(access_token, page=1, per_page=100)
        ...
        try:
            ...
            if not project:
                ...
                project = await self._fetch_project_from_github(
                    access_token, project_id, user_id
                )
    ...
    async def _fetch_project_from_github(
        ...
        try:
            ...
            tasks = [
                github_service.get_repository_details(access_token, owner, repo),
                github_service.get_repository_branches(access_token, owner, repo),
                github_service.get_repository_issues(access_token, owner, repo, state='open', page=1, per_page=50),
                github_service.get_repository_pull_requests(access_token, owner, repo, state='open', page=1, per_page=50),
                github_service.get_repository_commits(access_token, owner, repo, branch='main', page=1, per_page=50)
            ]
        ...
        try:
            ...
            if project and project.repository_url:
                ...
                try:
                    ...
                    branches = await github_service.get_repository_branches(access_token, owner, repo)
        ...
        try:
            ...
            if project and project.repository_url:
                ...
                try:
                    ...
                    tasks = [
                        github_service.get_repository_issues(access_token, owner, repo, state='all', page=1, per_page=100),
                        github_service.get_repository_pull_requests(access_token, owner, repo, state='all', page=1, per_page=100),
                        github_service.get_repository_commits(access_token, owner, repo, branch='main', page=1, per_page=100)
                    ]
    ...
    async def get_beetle_project_data(
        ...
        try:
            ...
            branches = await github_service.get_repository_branches(access_token, owner, repo)
            issues = await github_service.get_repository_issues(access_token, owner, repo, state='all', page=1, per_page=100)
            pull_requests = await github_service.get_repository_pull_requests(access_token, owner, repo, state='all', page=1, per_page=100)
            ...
            for branch in branches:
                branch_commits = await github_service.get_repository_commits(
                    access_token, owner, repo, branch=branch.get('name'), page=1, per_page=100
                )
    ...
    async def get_smart_suggestions(
        ...
        try:
            ...
            tasks = [
                ...
                github_service.get_repository_issues(access_token, owner, repo, state='all', page=1, per_page=100),
                github_service.get_repository_pull_requests(access_token, owner, repo, state='all', page=1, per_page=100),
                github_service.get_repository_commits(access_token, owner, repo, branch=branch, page=1, per_page=100)
            ]
...
async def list_user_projects(access_token: str, user_id: str) -> List[ProjectListItem]:
    """List user projects"""
    return await project_service.list_user_projects(access_token, user_id)

async def get_project_details(access_token: str, project_id: str, user_id: str) -> Optional[Project]:
    """Get project details"""
    return await project_service.get_project_details(access_token, project_id, user_id)
...
async def get_project_branches(access_token: str, project_id: str) -> List[ProjectBranch]:
    """Get project branches"""
    return await project_service.get_project_branches(access_token, project_id)

async def get_project_analytics(access_token: str, project_id: str) -> Dict[str, Any]:
    """Get project analytics"""
    return await project_service.get_project_analytics(access_token, project_id)

async def import_repository(access_token: str, import_request: RepositoryImportRequest, user_id: str) -> Project:
    """Import repository"""
    return await project_service.import_repository(access_token, import_request, user_id)

async def get_beetle_project_data(access_token: str, project_id: str) -> Optional[BeetleProjectData]:
    """Get Beetle project data"""
    return await project_service.get_beetle_project_data(access_token, project_id)

async def get_smart_suggestions(access_token: str, project_id: str, branch: str) -> List[SmartSuggestion]:
    """Get smart suggestions"""
    return await project_service.get_smart_suggestions(access_token, project_id, branch)

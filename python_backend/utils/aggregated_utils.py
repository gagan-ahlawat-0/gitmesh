...
class AggregatedDataService:
    ...
    async def get_aggregated_pull_requests(
        ...
        try:
            ...
            repositories = await github_service.get_user_repositories(access_token, page=1, per_page=limit)
            ...
            for repo in repositories:
                task = self._fetch_repository_prs_with_context(
                    semaphore, access_token, repo, state
                )
                tasks.append(task)
    ...
    async def _fetch_repository_prs_with_context(
        ...
        async with semaphore:
            try:
                ...
                # Fetch pull requests
                prs = await github_service.get_repository_pull_requests(
                    access_token, owner, repo_name, state, page=1, per_page=20
                )
    ...
    async def get_activity_summary(
        ...
        try:
            ...
            for repo in repositories[:min(5, len(repositories))]:  # Limit to first 5 repos
                task = self._fetch_repository_counts(semaphore, access_token, repo)
    ...
    async def _fetch_repository_counts(
        ...
        async with semaphore:
            try:
                ...
                tasks = [
                    github_service.get_repository_pull_requests(access_token, owner, repo_name, 'open', 1, 1),
                    github_service.get_repository_issues(access_token, owner, repo_name, 'open', 1, 1)
                ]
...
async def get_aggregated_pull_requests(access_token: str, limit: int = 10, state: str = 'all') -> Tuple[List[AggregatedPullRequest], Dict[str, Any]]:
    """Get aggregated pull requests"""
    return await aggregated_service.get_aggregated_pull_requests(access_token, limit, state)

async def get_aggregated_issues(access_token: str, limit: int = 10, state: str = 'all') -> Tuple[List[AggregatedIssue], Dict[str, Any]]:
    """Get aggregated issues"""
    return await aggregated_service.get_aggregated_issues(access_token, limit, state)

async def get_activity_summary(access_token: str, limit: int = 10) -> ActivitySummary:
    """Get activity summary"""
    return await aggregated_service.get_activity_summary(access_token, limit)

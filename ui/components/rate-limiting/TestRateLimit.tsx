...
const TestRateLimit: React.FC = () => {
  ...
  const testRateLimit = async () => {
    ...
    try {
      ...
      for (let i = 0; i < 5; i++) {
        setStatus(`Making request ${i + 1}/5...`);
      ...
      setStatus('All requests completed successfully');
    } catch (err: any) {
  ...
  const simulateRateLimit = () => {
    // Simulate a rate limit error by emitting the event directly
      ...
      <RateLimitHandler>
        <div className="max-w-2xl mx-auto p-6 space-y-6">
          <div className="bg-white rounded-lg shadow-md p-6">
            ...
            <RateLimitMonitor className="mb-4" />
          ...
          <div className="bg-gray-50 rounded-lg p-4">
            ...
            <ol className="list-decimal list-inside space-y-1 text-sm text-gray-700">
              <li>Click "Simulate Rate Limit" to see the rate limit handler</li>
              <li>Observe the countdown timer and user interface</li>
              <li>Click "Clear Rate Limit" to reset the state</li>
              <li>Click "Test Rate Limit" to make actual API calls (may fail due to authentication)</li>
            </ol>
          </div>
        </div>
      </RateLimitHandler>

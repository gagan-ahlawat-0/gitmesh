...
from .knowledge import Knowledge, GitMeshMemory


class Session:
    """
    A simple wrapper around existing stateful capabilities.
    ...
    def __init__(
        ...
        if not self.is_remote:
            # Use GitMeshMemory (Supabase + Qdrant hybrid) by default
            default_memory_config = {
                "provider": "supabase",  # Use Supabase + Qdrant hybrid
                "use_embedding": True,   # Enable free embeddings
            ...
            os.makedirs(f".gitmesh/sessions/{self.session_id}", exist_ok=True)

            # Initialize components lazily
            self._memory = None
            self._knowledge = None
            self._gitmesh_memory = None  # For hybrid GitMeshMemory
    ...
    @property
    def gitmesh_memory(self) -> GitMeshMemory:
        """Lazy-loaded GitMeshMemory instance (Supabase + Qdrant hybrid)"""
        if self.is_remote:
            raise ValueError("GitMeshMemory operations are not available for remote agent sessions")
        if self._gitmesh_memory is None:
            self._gitmesh_memory = GitMeshMemory(self.memory_config)
        return self._gitmesh_memory
    ...
    def add_memory(self, text: str, memory_type: str = "long", use_gitmesh: bool = True, **metadata) -> None:
        """
        ...
        Args:
            ...
            use_gitmesh: Whether to use GitMeshMemory (Supabase+Qdrant) or legacy Memory
        ...
        if use_gitmesh and self.memory_config.get("provider") == "supabase":
            # Use GitMeshMemory (Supabase + Qdrant hybrid)
            self.gitmesh_memory.add(text, user_id=self.user_id, metadata=metadata)
        else:
    ...
    def search_memory(self, query: str, memory_type: str = "long", limit: int = 5, use_gitmesh: bool = True) -> List[Dict[str, Any]]:
        """
        ...
        if use_gitmesh and self.memory_config.get("provider") == "supabase":
            # Use GitMeshMemory (Supabase + Qdrant hybrid)
            return self.gitmesh_memory.search(query, user_id=self.user_id, limit=limit)
        else:
            # Use legacy Memory system
            if memory_type == "short":

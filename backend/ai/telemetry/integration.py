"""
Performance-optimized integration module for adding telemetry to core GitMesh components.
...
def instrument_workflow(workflow: 'GitMeshAgents', telemetry: Optional['MinimalTelemetry'] = None, performance_mode: bool = False):
    """
    Instrument a AI workflow with performance-optimized telemetry.
    
    Args:
        workflow: The AI instance to instrument
        telemetry: Optional telemetry instance (uses global if not provided)
        performance_mode: If True, uses minimal overhead tracking
    """
    # Early exit if telemetry is disabled by environment variables
...
def auto_instrument_all(telemetry: Optional['MinimalTelemetry'] = None, performance_mode: bool = False):
    """
    Automatically instrument all new instances of Agent and GitMeshAgents with optimized telemetry.
    ...
    try:
        ...
        if explicitly_disabled:
            ...
            explicitly_enabled = any([
                os.environ.get('GITMESH_PERFORMANCE_ENABLED', '').lower() in ('true', '1', 'yes'),
                os.environ.get('GITMESH_TELEMETRY_ENABLED', '').lower() in ('true', '1', 'yes'),
            ])
    ...
    try:
        ...
        original_workflow_init = ai.__init__
        ...
        ai.__init__ = workflow_init_wrapper
...
def cleanup_telemetry_resources():
    ...
    if _telemetry_executor:
        try:
            import os
            shutdown_timeout = float(os.environ.get('GITMESH_TELEMETRY_SHUTDOWN_TIMEOUT', '5.0'))

...
Usage:
    python -m ai.telemetry.performance_cli [command] [options]
    
Or import and use programmatically:
    from ai.telemetry.performance_cli import PerformanceCLI
    cli = PerformanceCLI()
...
class PerformanceCLI:
    ...
    def __init__(self):
        if not PERFORMANCE_TOOLS_AVAILABLE:
            print("x Performance monitoring tools not available")
            sys.exit(1)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  python -m ai.telemetry.performance_cli report
  python -m ai.telemetry.performance_cli functions
  python -m ai.telemetry.performance_cli apis
  python -m ai.telemetry.performance_cli slowest-functions 5
  python -m ai.telemetry.performance_cli flow
  python -m ai.telemetry.performance_cli analyze-flow
  python -m ai.telemetry.performance_cli trends
        """
    )

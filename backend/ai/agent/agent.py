    ...
    def __init__(
        ...
        Args:
            ...
            user_id (Optional[str], optional): Unique identifier for the user or session to
                enable personalized responses and memory isolation. Defaults to "gitmesh".
...
Your Goal: {self.goal}
        ...
        # Store user_id
        self.user_id = user_id or "gitmesh"
        ...
        if protocol == "http":
            ...
            if _shared_apps.get(port) is None:
                _shared_apps[port] = FastAPI(
                    title=f"GitMesh Agents API (Port {port})",
                    description="API for interacting with GitMesh Agents"
                )
                ...
                async def root():
                    return {
                        "message": f"Welcome to GitMesh Agents API on port {port}. See /docs for usage.",
            ...
            try:
                ...
                print(f"pip install {missing_module} mcp gitmesh-mcp starlette uvicorn") # Added mcp, gitmesh-mcp, starlette, uvicorn

...
class GitMeshAgents:
    def __init__(self, agents, tasks=None, verbose=0, completion_checker=None, max_retries=5, process="sequential", manager_llm=None, memory=False, memory_config=None, embedder=None, user_id=None, max_iter=10, stream=True, name: Optional[str] = None):
        ...
        if memory:
            try:
                ...
                raise ImportError(
                    ...
                    "Please install with: pip install \"gitmesh[memory]\""
                )
        ...
        self._state = {}  # Add state storage at GitMeshAgents level
        ...
        if memory:
            try:
                ...
                if mem_cfg:
                    ...
                    if verbose >= 5:
                        logger.info("Initialized shared memory for GitMeshAgents")
        ...
        if protocol == "http":
            ...
            if _agents_shared_apps.get(port) is None:
                _agents_shared_apps[port] = FastAPI(
                    title=f"GitMesh AI Agents API (Port {port})",
                    description="API for interacting with multiple GitMesh AI Agents"
                )
                ...
                async def root():
                    return {
                        "message": f"Welcome to GitMesh AI Agents API on port {port}. See /docs for usage.",
            ...
            mcp_instance = FastMCP("gitmesh_workflow_mcp_server")
            ...
            print(f"🚀 GitMesh Agents MCP Workflow server starting on http://{host}:{port}")
            ...
            def run_praison_mcp_server():
                try:
                    ...
                    logging.error(f"Error starting GitMesh Agents MCP server: {str(e)}", exc_info=True)
                    print(f"❌ Error starting GitMesh Agents MCP server: {str(e)}")
            ...
            if len(stack) > 1 and stack[1].filename.endswith('.py'):
                ...
                try:
                    ...
                    if not has_more_launches:
                        try:
                            print("\nGitMesh Agents MCP server running. Press Ctrl+C to stop.")
                            while True:
                                time.sleep(1)
                        except KeyboardInterrupt:
                            print("\nGitMesh Agents MCP Server stopped")
                except Exception as e:
                    logging.error(f"Error in GitMesh Agents MCP launch detection: {e}")
                    try:
                        print("\nKeeping GitMesh Agents MCP server alive. Press Ctrl+C to stop.")
                        while True:
                            time.sleep(1)
                    except KeyboardInterrupt:
                        print("\nGitMesh Agents MCP Server stopped")

"""
GitMesh AI Agents - A package for hierarchical AI agent task execution
"""
...
from .agents.agents import GitMeshAgents
...
# Add Agents as an alias for GitMeshAgents
Agents = GitMeshAgents
...
# Users can:
#   - Disable completely: GITMESH_DISABLE_TELEMETRY=true (or DO_NOT_TRACK=true)
#   - Enable performance mode: GITMESH_PERFORMANCE_MODE=true (minimal overhead, limited events)
#   - Enable full telemetry: GITMESH_FULL_TELEMETRY=true (detailed tracking)
#   - Legacy opt-in mode: GITMESH_AUTO_INSTRUMENT=true
if _telemetry_available:
    try:
        ...
        is_disabled = any([
            ...
            os.environ.get('GITMESH_TELEMETRY_DISABLED', '').lower() in ('true', '1', 'yes'),
            os.environ.get('GITMESH_DISABLE_TELEMETRY', '').lower() in ('true', '1', 'yes'),
        ])
        
        # Determine telemetry mode
        performance_mode = os.environ.get('GITMESH_PERFORMANCE_MODE', '').lower() in ('true', '1', 'yes')
        
        # Full telemetry mode - detailed tracking
        full_telemetry = os.environ.get('GITMESH_FULL_TELEMETRY', '').lower() in ('true', '1', 'yes')
        
        # Legacy auto-instrument mode
        explicit_auto_instrument = os.environ.get('GITMESH_AUTO_INSTRUMENT', '').lower() in ('true', '1', 'yes')

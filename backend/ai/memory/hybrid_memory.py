"""
Hybrid memory implementation using Qdrant Cloud for vector storage
and Supabase PostgreSQL for metadata and structured storage.
"""
...
try:
    # Check for Qdrant dependencies
    from .qdrant_db import QdrantMemory
...
class Memory:
    ...
    def __init__(self, config: Dict[str, Any], verbose: int = 0):
        ...
        self.provider = self.cfg.get("provider", "hybrid")
        self.vector_provider = self.cfg.get("vector_provider", os.getenv("VECTOR_PROVIDER", "qdrant"))
        
        # Always use the hybrid approach with Supabase and Qdrant
        self.use_supabase = SUPABASE_AVAILABLE
        self.use_qdrant = QDRANT_AVAILABLE and self.vector_provider.lower() == "qdrant"
        ...
        # Initialize Supabase for metadata and structured storage
        if self.use_supabase:
            self._log_verbose("Initializing Supabase PostgreSQL for structured data storage")
            self._init_supabase()
        else:
        ...
        if self.use_qdrant:
            self._log_verbose("Initializing Qdrant for vector storage")
            self._init_qdrant()
        else:
            self._log_verbose("WARNING: Qdrant is not available, falling back to Supabase pgvector", logging.WARNING)
    ...
    def _init_supabase(self):
        """Initialize Supabase PostgreSQL for structured storage."""
        try:
    ...
    def _init_qdrant(self):
        """Initialize Qdrant for vector storage."""
        try:
            # Get Qdrant configuration from environment variables or config
            qdrant_config = self.cfg.get("qdrant_config", {})
            
            # Create the Qdrant Memory instance
            self.qdrant_memory = QdrantMemory(
                config=qdrant_config,
                verbose=self.verbose
            )
            
            self._log_verbose("Qdrant vector storage initialized successfully")
            
        except Exception as e:
            self._log_verbose(f"Failed to initialize Qdrant: {e}", logging.ERROR)
            self.use_qdrant = False
            # Fall back to using Supabase pgvector
            self._log_verbose("Falling back to Supabase pgvector for vector storage")

    def _get_embedding(self, text: str) -> List[float]:
        """
        Get embedding for text using available embedding services.
        This method is used by vector databases for similarity search.
        """
        try:
    ...
    def store_short_term(
        ...
        # Store in appropriate storage based on availability and configuration
        if self.use_qdrant:
            try:
                # First store in Qdrant for vector search
                memory_id = self.qdrant_memory.store_short_term(
                    text=text,
                    metadata=metadata
                )
                
                # Then store in Supabase for structured data
                self.supabase_memory.store_short_term(
                    text=text,
                    metadata=metadata,
                    memory_id=memory_id  # Use the same ID for consistency
                )
                
                logger.info(f"Successfully stored in hybrid short-term memory with ID: {memory_id}")
                return memory_id
            except Exception as e:
                logger.error(f"Failed to store in Qdrant short-term memory: {e}")
                # Fall back to Supabase only
        
        # Use Supabase for storage (either as primary or fallback)
        if self.use_supabase:
            try:
            ...
            raise RuntimeError("No memory storage is available")
    ...
    def search_short_term(
        ...
        **kwargs
        ...
        if self.use_qdrant:
            try:
                results = self.qdrant_memory.search_short_term(
                    query=query,
                    limit=limit,
                ...
                self._log_verbose(f"Error searching Qdrant short-term memory: {e}", logging.ERROR)
                # Fall back to Supabase
        
        # Use Supabase for search (either as primary or fallback)
        if self.use_supabase:
            try:
    ...
    def reset_short_term(self):
        """Reset short-term memory in all storage systems"""
        if self.use_qdrant:
            try:
                self.qdrant_memory.reset_short_term()
                self._log_verbose("Successfully reset short-term memory in Qdrant")
            except Exception as e:
                self._log_verbose(f"Error resetting short-term memory in Qdrant: {e}", logging.ERROR)
    ...
    def _sanitize_metadata(self, metadata: Dict) -> Dict:
        """Sanitize metadata for storage - convert to acceptable types"""
        sanitized = {}
    ...
    def store_long_term(
        ...
        # Store in appropriate storage based on availability
        if self.use_qdrant:
            try:
                # First store in Qdrant for vector search
                memory_id = self.qdrant_memory.store_long_term(
                    text=text,
                    metadata=metadata
                )
                
                # Then store in Supabase for structured data
                self.supabase_memory.store_long_term(
                    text=text,
                    metadata=metadata,
                    memory_id=memory_id  # Use the same ID for consistency
                )
                
                logger.info(f"Successfully stored in hybrid long-term memory with ID: {memory_id}")
                return memory_id
            except Exception as e:
                logger.error(f"Failed to store in Qdrant long-term memory: {e}")
                # Fall back to Supabase only
    ...
    def search_long_term(
        ...
        **kwargs
        ...
        if self.use_qdrant:
            try:
                results = self.qdrant_memory.search_long_term(
                    query=query,
                    limit=limit,
                    min_quality=min_quality,
                    relevance_cutoff=relevance_cutoff,
                ...
                self._log_verbose(f"Error searching Qdrant long-term memory: {e}", logging.ERROR)
                # Fall back to Supabase
    ...
    def reset_long_term(self):
        """Reset long-term memory in all storage systems"""
        if self.use_qdrant:
            try:
                self.qdrant_memory.reset_long_term()
                self._log_verbose("Successfully reset long-term memory in Qdrant")
            except Exception as e:
                self._log_verbose(f"Error resetting long-term memory in Qdrant: {e}", logging.ERROR)
    ...
    def store_entity(self, name: str, type_: str, desc: str, relations: str):
        """
        Save entity info in memory storage
        """
        # Use Qdrant for entity storage if available
        if self.use_qdrant:
            try:
                # First store in Qdrant for vector search
                entity_id = self.qdrant_memory.store_entity(
                    name=name,
                    type_=type_,
                    desc=desc,
                    relations=relations
                ...
                # Then store in Supabase for structured data
                self.supabase_memory.store_entity(
                    name=name,
                    type_=type_,
                    desc=desc,
                    relations=relations,
                    entity_id=entity_id  # Use the same ID for consistency
                )
                
                self._log_verbose(f"Successfully stored entity in hybrid storage with ID: {entity_id}")
                return entity_id
            except Exception as e:
                self._log_verbose(f"Error storing entity in Qdrant: {e}", logging.ERROR)
                # Fall back to Supabase only
    ...
    def search_entity(
        ...
        # Use Qdrant for entity search if available
        if self.use_qdrant:
            try:
                results = self.qdrant_memory.search_entity(
                    query=query,
                ...
                self._log_verbose(f"Error searching Qdrant entity memory: {e}", logging.ERROR)
                # Fall back to Supabase
    ...
    def reset_entity_only(self):
        """
        Reset only entity data in storage
        """
        if self.use_qdrant:
            try:
                self.qdrant_memory.reset_entities()
                self._log_verbose("Successfully reset entity memory in Qdrant")
            except Exception as e:
                self._log_verbose(f"Error resetting entity memory in Qdrant: {e}", logging.ERROR)
    ...
    def store_user_memory(self, user_id: str, text: str, extra: Dict[str, Any] = None):
        """
        Store user-specific memory in storage
        """
        meta = {"user_id": user_id}
        if extra:
            meta.update(extra)

        # Use Qdrant for user memory storage if available
        if self.use_qdrant:
            try:
                # First store in Qdrant for vector search
                memory_id = self.qdrant_memory.store_user_memory(
                    user_id=user_id,
                    text=text,
                    metadata=meta
                )
                ...
                # Then store in Supabase for structured data
                self.supabase_memory.store_user_memory(
                    user_id=user_id,
                    text=text,
                    metadata=meta,
                    memory_id=memory_id  # Use the same ID for consistency
                ...
                self._log_verbose(f"Successfully stored user memory in hybrid storage for {user_id}")
                return memory_id
            except Exception as e:
                self._log_verbose(f"Error storing user memory in Qdrant: {e}", logging.ERROR)
                # Fall back to Supabase only
    ...
    def search_user_memory(self, user_id: str, query: str, limit: int = 5, rerank: bool = False, **kwargs) -> List[Dict[str, Any]]:
        """
        Search user memory in storage
        """
        self._log_verbose(f"Searching user memory for user {user_id} with query: {query}")
        
        # Use Qdrant for user memory search if available
        if self.use_qdrant:
            try:
                results = self.qdrant_memory.search_user_memory(
                    user_id=user_id,
                    query=query,
                    limit=limit,
                    rerank=rerank
                ...
                self._log_verbose(f"Error searching user memory in Qdrant: {e}", logging.ERROR)
                # Fall back to Supabase
    ...
    def search(self, query: str, user_id: Optional[str] = None, agent_id: Optional[str] = None, 
        ...
        """
        # Use Qdrant for generic search if available
        if self.use_qdrant:
            try:
                ...
                self._log_verbose(f"Error in generic search with Qdrant: {e}", logging.ERROR)
                # Fall back to Supabase
    ...
    def reset_user_memory(self):
        """
        Reset only user memory in storage
        """
        if self.use_qdrant:
            try:
                self.qdrant_memory.reset_user_memory()
                self._log_verbose("Successfully reset user memory in Qdrant")
            except Exception as e:
                self._log_verbose(f"Error resetting user memory in Qdrant: {e}", logging.ERROR)
    ...
    def build_context_for_task(
        ...
        """
        # Ensure at least one storage system is available
        if not (self.use_supabase or self.use_qdrant):
            self._log_verbose("ERROR: No memory storage is available for context building", logging.ERROR)
            return ""
        ...
        try:
            # Add each section
            # First get all results from available storage
            if self.use_qdrant:
                # Prefer Qdrant for vector search
                short_term = self.qdrant_memory.search_short_term(q, limit=max_items)
                long_term = self.qdrant_memory.search_long_term(q, limit=max_items)
                entities = self.qdrant_memory.search_entity(q, limit=max_items)
                user_mem = self.qdrant_memory.search_user_memory(user_id, q, limit=max_items) if user_id else []
            else:
                # Fall back to Supabase
                short_term = self.supabase_memory.search_short_term(q, limit=max_items)
                long_term = self.supabase_memory.search_long_term(q, limit=max_items)
                entities = self.supabase_memory.search_entity(q, limit=max_items)
                user_mem = self.supabase_memory.search_user_memory(user_id, q, limit=max_items) if user_id else []
            ...
            self._log_verbose(f"Error building context: {e}", logging.ERROR)
            return ""
    ...
    def reset_all(self):
        """
        Fully wipes memory in all storage systems
        """
        if self.use_qdrant:
            try:
                self.qdrant_memory.reset_all()
                self._log_verbose("Successfully reset all memory in Qdrant")
            except Exception as e:
                self._log_verbose(f"Error resetting all memory in Qdrant: {e}", logging.ERROR)
    ...
    def search_with_quality(
        ...
        results = search_func(query, limit=limit, min_quality=min_quality)
        logger.info(f"Found {len(results)} results after quality filter")
        ...
        try:
            ...
            if self.use_qdrant:
                try:
                    ...
                    self._log_verbose(f"Error getting all memories from Qdrant: {e}", logging.ERROR)
            
            # Fall back to Supabase
            if self.use_supabase:
                return self.supabase_memory.get_all_memories()

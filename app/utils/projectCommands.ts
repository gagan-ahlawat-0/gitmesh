...
export function createCommandsMessage(commands: ProjectCommands): Message | null {
  ...
  if (commands.setupCommand) {
    commandString += `
<gitmeshAction type="shell">${commands.setupCommand}</gitmeshAction>`;
  }

  if (commands.startCommand) {
    commandString += `
<gitmeshAction type="start">${commands.startCommand}</gitmeshAction>
`;
  }
...
<gitmeshArtifact id="project-setup" title="Project Setup">
${commandString}
</gitmeshArtifact>`,
    id: generateId(),
    createdAt: new Date(),
...
export function escapegitmeshArtifactTags(input: string) {
  // Safety check to ensure input is a string
  if (typeof input !== 'string') {
    console.warn('escapegitmeshArtifactTags called with non-string input:', typeof input, input);
    return String(input || '');
  }

  // Regular expression to match gitmeshArtifact tags and their content
  const regex = /(<gitmeshArtifact[^>]*>)([\s\S]*?)(<\/gitmeshArtifact>)/g;
...
export function escapegitmeshAActionTags(input: string) {
  // Safety check to ensure input is a string
  if (typeof input !== 'string') {
    console.warn('escapegitmeshAActionTags called with non-string input:', typeof input, input);
    return String(input || '');
  }

  // Regular expression to match gitmeshArtifact tags and their content
  const regex = /(<gitmeshAction[^>]*>)([\s\S]*?)(<\/gitmeshAction>)/g;
...
export function escapegitmeshTags(input: string) {
  // Safety check to ensure input is a string
  if (typeof input !== 'string') {
    console.warn('escapegitmeshTags called with non-string input:', typeof input, input);
    return String(input || '');
  }

  return escapegitmeshArtifactTags(escapegitmeshAActionTags(input));
}

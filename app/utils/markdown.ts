  ...
  return (tree: any) => {
    visit(tree, (node: any) => {
      if (node.type === 'html' && node.value && node.value.startsWith('<think>')) {
        ...
        node.value = `<div class="__gitmeshThought__">${cleanedContent}`;
...
const rehypeSanitizeOptions: RehypeSanitizeOptions = {
  ...
  attributes: {
    ...
    div: [
      ...
      'data*',
      ['className', '__gitmeshArtifact__', '__gitmeshThought__', '__gitmeshQuickAction', '__gitmeshSelectedElement__'],

      // ['className', '__gitmeshThought__']
    ],
    button: [
      ...
      'name',
      'value',
      ['className', '__gitmeshArtifact__', '__gitmeshThought__', '__gitmeshQuickAction'],
    ],
  },
  strip: [],
};

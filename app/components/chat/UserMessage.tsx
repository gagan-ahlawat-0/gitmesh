...
export function UserMessage({ content, parts }: UserMessageProps) {
  ...
  if (Array.isArray(content)) {
    ...
    return (
      <div className="overflow-hidden flex flex-col gap-3 items-center ">
          ...
          {profile?.avatar || profile?.username ? (
            <div className="flex items-end gap-2">
              <img
                ...
                decoding="sync"
              />
              <span className="text-gitmesh-elements-textPrimary text-sm">
        ...
        <div className="flex flex-col gap-4 bg-blue-500/10 backdrop-blur-sm p-3 py-3 w-auto rounded-lg mr-auto">
  ...
  return (
    <div className="flex flex-col bg-blue-500/10 backdrop-blur-sm px-5 p-3.5 w-auto rounded-lg ml-auto">
      <div className="flex gap-3.5 mb-4">
        {images.map((item, index) => (
          <div className="relative flex rounded-lg border border-gitmesh-elements-borderColor overflow-hidden">
...
function stripMetadata(content: string) {
  const artifactRegex = /<gitmeshArtifact\s+[^>]*>[\s\S]*?<\/gitmeshArtifact>/gm;

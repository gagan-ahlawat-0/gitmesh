    ...
    <div className="flex items-center gap-1">
      ...
      {shouldShowButtons && (
        <div className="flex border border-gitmesh-elements-borderColor rounded-md overflow-hidden text-sm">
          <button
            onClick={() =>
              window.open('https://github.com/stackblitz-labs/gitmesh.diy/issues/new?template=bug_report.yml', '_blank')
            }
          ...
          <div className="w-px bg-gitmesh-elements-borderColor" />
          <button
            onClick={async () => {
              try {
            ...
            className="rounded-r-md items-center justify-center [&:is(:disabled,.disabled)]:cursor-not-allowed [&:is(:disabled,.disabled)]:opacity-60 px-3 py-1.5 text-xs bg-blue-500 text-white hover:text-gitmesh-elements-item-contentAccent [&:not(:disabled,.disabled)]:hover:bg-gitmesh-elements-button-primary-backgroundHover outline-accent-500 flex gap-1.5"

    ...
    return (
      <div className={classNames('flex flex-col items-center justify-center py-8', className)}>
        ...
        <div className="text-center space-y-2">
          <p className="text-sm font-medium text-gitmesh-elements-textPrimary">{loadingMessage}</p>

          {showProgress && progressSteps.length > 0 && (
            <div className="w-full max-w-sm">
              {/* Progress bar */}
              <div className="w-full bg-gitmesh-elements-background-depth-1 rounded-full h-2 mb-3">
                <motion.div
                  className="bg-gitmesh-elements-item-contentAccent h-2 rounded-full"
                  initial={{ width: 0 }}
              ...
              <button
                ...
                className="flex items-center justify-center gap-2 text-xs text-gitmesh-elements-textSecondary hover:text-gitmesh-elements-textPrimary transition-colors"
              >
                    ...
                    {progressSteps.map((step) => (
                      <div key={step.key} className="flex items-center gap-2 text-xs">
                        {step.error ? (
                          ...
                          <div className="w-3 h-3 rounded-full border border-gitmesh-elements-borderColor flex-shrink-0" />
                        )}
                        <span
                          className={classNames(
                            step.error
  ...
  if (error) {
    return (
      <div className={classNames('flex flex-col items-center justify-center py-8 text-center space-y-4', className)}>
        ...
        <div>
          <h3 className="text-sm font-medium text-gitmesh-elements-textPrimary mb-1">Failed to Load</h3>
          <p className="text-xs text-gitmesh-elements-textSecondary mb-4 max-w-sm">{error}</p>
        </div>
    ...
    <div className={classNames('relative', className)}>
      {isRefreshing && (
        <div className="absolute top-0 right-0 z-10">
          <div className="flex items-center gap-2 px-2 py-1 bg-gitmesh-elements-background-depth-1 border border-gitmesh-elements-borderColor rounded-lg shadow-sm">
            <Loader2 className="w-3 h-3 animate-spin text-blue-500" />
            <span className="text-xs text-gitmesh-elements-textSecondary">{refreshingMessage}</span>
          </div>
        </div>
      )}

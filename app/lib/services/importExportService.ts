...
export class ImportExportService {
    ...
    try {
      ...
      return {
        ...
        core: {
          ...
          gitmesh_user_profile: this._safeGetItem('gitmesh_user_profile'),
          gitmesh_settings: this._safeGetItem('gitmesh_settings'),
          gitmesh_profile: this._safeGetItem('gitmesh_profile'),
        ...
        features: {
          // Feature flags
          viewed_features: this._safeGetItem('gitmesh_viewed_features'),
          developer_mode: this._safeGetItem('gitmesh_developer_mode'),
        ...
        ui: {
          // Tab configuration
          gitmesh_tab_configuration: this._safeGetItem('gitmesh_tab_configuration'),
        ...
        debug: {
          ...
          acknowledged_debug_issues: this._safeGetItem('gitmesh_acknowledged_debug_issues'),
          acknowledged_connection_issue: this._safeGetItem('gitmesh_acknowledged_connection_issue'),

          // Error logs
          error_logs: this._safeGetItem('error_logs'),
          gitmesh_read_logs: this._safeGetItem('gitmesh_read_logs'),
        ...
        updates: {
          ...
          last_acknowledged_update: this._safeGetItem('gitmesh_last_acknowledged_version'),
        },
  ...
  static async deleteAllChats(db: IDBDatabase): Promise<void> {
    ...
    localStorage.removeItem('gitmesh_chat_history');
  ...
  private static async _importComprehensiveFormat(data: any): Promise<void> {
    ...
    if (data.ui) {
      ...
      if (data.ui.gitmesh_tab_configuration) {
        try {
          this._safeSetItem('gitmesh_tab_configuration', data.ui.gitmesh_tab_configuration);
        } catch (err) {
    ...
    if (data.debug) {
      ...
      const debugLocalStorageKeys = [
        'gitmesh_acknowledged_debug_issues',
        'gitmesh_acknowledged_connection_issue',
        'error_logs',
        'gitmesh_read_logs',
      ];
    ...
    if (data.updates) {
      ...
      if (data.updates.last_acknowledged_update) {
        try {
          this._safeSetItem('gitmesh_last_acknowledged_version', data.updates.last_acknowledged_update);
        } catch (err) {

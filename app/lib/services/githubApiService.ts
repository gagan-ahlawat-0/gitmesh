...
export class GitHubApiServiceClass {
  ...
  /**
   * Calculate basic repository statistics without detailed API calls
   */
  calculateBasicRepositoryStats(repos: GitHubRepoInfo[]): {
    ...
    repos.forEach((repo) => {
      ...
      // Repository health analysis (based on available data)
      ...
      if (repo.archived) {
        ...
        activeRepos++;
      } else if (daysSinceUpdate < 30 && (repo.stargazers_count || 0) > 0) {
        healthyRepos++;
      }
    });
  ...
  async generateComprehensiveStats(userData: GitHubUserResponse): Promise<GitHubStats> {
    try {
      ...
      /*
       * Limit detailed analysis to prevent API rate limiting and performance issues
       * Only get detailed info for top repositories (by stars and recent activity)
       */
      const topRepos = allRepos
        .sort((a, b) => {
          // Sort by stars first, then by updated_at
        ...
        .slice(0, 20); // Limit to top 20 repositories for detailed analysis

      // Get detailed information for limited repositories only (in batches)
      const detailedRepos = await this.getDetailedRepositoriesInfo(topRepos, 3, 200); // Slower batches

      // Use all repos for basic stats, detailed repos for advanced metrics
      const basicStats = this.calculateBasicRepositoryStats(allRepos);
      const detailedStats = this.calculateRepositoryStats(detailedRepos);
      ...
      // Calculate aggregated metrics from ALL repos for accurate totals
      ...
      const githubStats: GitHubStats = {
        repos: allRepos, // Include all repos in the stats, not just detailed ones
        recentActivity:
          recentActivity.status === 'fulfilled'
            ...
            : [],
        languages: basicStats.languages,
        ...
        // Use detailed stats for advanced metrics (only from top repos)
        totalBranches: detailedStats.totalBranches,
        totalContributors: detailedStats.totalContributors,
        totalIssues: detailedStats.totalIssues,
        totalPullRequests: detailedStats.totalPullRequests,
        mostUsedLanguages: basicStats.mostUsedLanguages,
      };

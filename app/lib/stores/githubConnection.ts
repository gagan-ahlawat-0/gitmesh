...
export const githubStatsUpdateTrigger = atom(0);
...
export const githubConnectionStore = {
  ...
  async fetchStats(): Promise<void> {
    ...
    // Check if we have recent stats (less than 5 minutes old)
    if (connection.stats?.lastUpdated) {
      const lastUpdate = new Date(connection.stats.lastUpdated);
      const fiveMinutesAgo = new Date(Date.now() - 5 * 60 * 1000);

      if (lastUpdate > fiveMinutesAgo) {
        return; // Stats are recent enough
      }
    }
    ...
    try {
      ...
      const timeoutPromise = new Promise((_, reject) => {
        setTimeout(() => reject(new Error('Stats fetching timeout')), 15000); // 15 second timeout
      });

      const statsPromise = gitHubApiService.fetchStats(connection.token, connection.tokenType);
      ...
      // Update atom - this should trigger reactivity
      githubConnectionAtom.set(updatedConnection);

      // Trigger update notification
      githubStatsUpdateTrigger.set(Date.now());
      ...
      if (error instanceof Error && error.message.includes('401')) {
        ...
        logStore.logError('GitHub stats loading timeout', {
          type: 'system',
          message: 'GitHub stats are taking too long to load. You can continue using the app.',
        });
      }

      // Don't throw error for timeout, just log it

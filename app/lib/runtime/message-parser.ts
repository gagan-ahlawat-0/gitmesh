...
import type { GitMeshArtifactData } from '~/types/artifact';
import { createScopedLogger } from '~/utils/logger';
import { unreachable } from '~/utils/unreachable';

const ARTIFACT_TAG_OPEN = '<gitmeshArtifact';
const ARTIFACT_TAG_CLOSE = '</gitmeshArtifact>';
const ARTIFACT_ACTION_TAG_OPEN = '<gitmeshAction';
const ARTIFACT_ACTION_TAG_CLOSE = '</gitmeshAction>';
const GITMESH_QUICK_ACTIONS_OPEN = '<gitmesh-quick-actions>';
const GITMESH_QUICK_ACTIONS_CLOSE = '</gitmesh-quick-actions>';

const logger = createScopedLogger('MessageParser');

export interface ArtifactCallbackData extends GitMeshArtifactData {
  messageId: string;
  artifactId?: string;
}
...
interface MessageState {
  ...
  currentArtifact?: GitMeshArtifactData;
  currentAction: GitMeshActionData;
  actionId: number;
}
...
export class StreamingMessageParser {
  ...
  parse(messageId: string, input: string) {
    ...
    while (i < input.length) {
      if (input.startsWith(GITMESH_QUICK_ACTIONS_OPEN, i)) {
        const actionsBlockEnd = input.indexOf(GITMESH_QUICK_ACTIONS_CLOSE, i);

        if (actionsBlockEnd !== -1) {
          const actionsBlockContent = input.slice(i + GITMESH_QUICK_ACTIONS_OPEN.length, actionsBlockEnd);

          // Find all <gitmesh-quick-action ...>label</gitmesh-quick-action> inside
          const quickActionRegex = /<gitmesh-quick-action([^>]*)>([\s\S]*?)<\/gitmesh-quick-action>/g;
          let match;
          const buttons = [];
          ...
          i = actionsBlockEnd + GITMESH_QUICK_ACTIONS_CLOSE.length;
          continue;
        }
      }
        ...
        if (state.insideAction) {
          ...
          if (closeIndex !== -1) {
            ...
            this._options.callbacks?.onActionClose?.({
              ...
              action: currentAction as GitMeshAction,
            });
          ...
          if (actionOpenIndex !== -1 && (artifactCloseIndex === -1 || actionOpenIndex < artifactCloseIndex)) {
            ...
            if (actionEndIndex !== -1) {
              ...
              this._options.callbacks?.onActionOpen?.({
                ...
                action: state.currentAction as GitMeshAction,
              });
...
function createQuickActionElement(props: Record<string, string>, label: string) {
  const elementProps = [
    'class="__gitmeshQuickAction__"',
    'data-gitmesh-quick-action="true"',
...
function createQuickActionGroup(buttons: string[]) {
  return `<div class=\"__gitmeshQuickAction__\" data-gitmesh-quick-action=\"true\">${buttons.join('')}</div>`;
}

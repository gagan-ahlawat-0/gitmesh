...
export class EnhancedStreamingMessageParser extends StreamingMessageParser {
  ...
  private _hasDetectedArtifacts(input: string): boolean {
    return input.includes('<gitmeshArtifact') || input.includes('</gitmeshArtifact>');
  }
  ...
  private _wrapInArtifact(artifactId: string, filePath: string, content: string): string {
    ...
    return `<gitmeshArtifact id="${artifactId}" title="${title}" type="bundled">
<gitmeshAction type="file" filePath="${filePath}">
${content}
</gitmeshAction>
</gitmeshArtifact>`;
  }
  ...
  private _wrapInShellAction(content: string, messageId: string): string {
    ...
    return `<gitmeshArtifact id="${artifactId}" title="Shell Command" type="shell">

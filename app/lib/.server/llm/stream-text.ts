...
function sanitizeText(text: string): string {
  ...
  if (typeof text !== 'string') {
    console.warn('sanitizeText called with non-string input:', typeof text, text);
    return String(text || '');
  }

  let sanitized = text.replace(/<div class=\\"__gitmeshThought__\\">.*?<\/div>/s, '');
  sanitized = sanitized.replace(/<think>.*?<\/think>/s, '');
  sanitized = sanitized.replace(
    /<gitmeshAction type="file" filePath="package-lock\.json">[\s\S]*?<\/gitmeshAction>/g,
    '',
  );
...
export async function streamText(props: {
  ...
  // Include context files for both build and discuss modes when available
  if (contextFiles && contextOptimization) {
    ...
    if (chatMode === 'build') {
      ...
      ---
      `;
    } else if (chatMode === 'discuss') {
      // For discuss mode, append the repository context to the discuss prompt
      systemPrompt = `${systemPrompt}

      You are currently discussing a project with the following codebase context. Use this information to provide accurate and relevant answers about the project:

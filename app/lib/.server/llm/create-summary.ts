...
import { extractCurrentContext, extractPropertiesFromMessage, simplifygitmeshActions } from './utils';
...
export async function createSummary(props: {
  ...
  const processedMessages = messages.map((message) => {
    if (message.role === 'user') {
      ...
      content = simplifygitmeshActions(content);
      content = content.replace(/<div class=\\"__gitmeshThought__\\">.*?<\/div>/s, '');

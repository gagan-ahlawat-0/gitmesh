...
export function useGitHubStats(
  ...
  useEffect(() => {
    if (autoFetch && connection && (!state.stats || isStale) && !state.isLoading && !state.isRefreshing) {
      /*
      ...
      if (isServerSide || apiService) {
        // Use a longer timeout to prevent immediate fetching on mount and reduce load
    ...
    return undefined;
  }, [autoFetch, connection, apiService, state.stats, isStale, isServerSide, state.isLoading, state.isRefreshing]);
